...
import com.carryjey.social.util.identicon.generator.IBaseGenartor;
import com.carryjey.social.util.identicon.generator.impl.MyGenerator;
...
public class Identicon {
    ...
    public String saveFile(String username, BufferedImage image) {
        ...
        try {
            ...
            File file1 =
                new File(
                    systemConfigService.selectAllConfig().get("uploadPath").toString() + userAvatarPath + fileName);
            if (!file1.exists()) {
                file1.createNewFile();
            }
    ...
    //      Auth auth = Auth.create(siteConfig.getUpload().getQiniu().getAccessKey(),
    // siteConfig.getUpload().getQiniu().getSecretKey());
