package com.carryjey.social.util.identicon.generator.impl;
...
public class DefaultGenerator implements IBaseGenartor {
    ...
    public boolean[][] getBooleanValueArray(String hash) {
        Preconditions.checkArgument(
            !StringUtils.isEmpty(hash) && hash.length() >= 16, "illegal argument hash:not null and size >= 16");
        ...
        for (int i = 0; i < hash.length(); i += 2) {
            ...
            boolean v =
                DoubleMath.roundToInt(Integer.parseInt(hash.charAt(i) + "", 16) / 10.0, RoundingMode.HALF_UP) > 0
                    ? true
                    : false;
            if (s % 3 == 0) {
                array[s / 3][0] = v;
